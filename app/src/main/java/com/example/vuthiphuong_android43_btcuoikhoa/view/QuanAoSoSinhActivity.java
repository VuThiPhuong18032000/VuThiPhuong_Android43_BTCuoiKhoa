...
public class QuanAoSoSinhActivity extends AppCompatActivity {
    RecyclerView rcvLoaiQASoSinh;
    AdapterSanPham adapterLoaiQASoSinh;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_quan_ao_so_sinh);
        rcvLoaiQASoSinh = findViewById(R.id.rcvLoaiQASoSinh);
        Button btnBackQASS = findViewById(R.id.btnBackQASS);
        RecyclerView.LayoutManager layoutManager = new GridLayoutManager(this,2,RecyclerView.VERTICAL,false);
        rcvLoaiQASoSinh.setLayoutManager(layoutManager);

        new DataGetPrduct().execute("https://demo7852048.mockable.io/SanPham");

        btnBackQASS.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                    samPhamNoiBatList.add(samPham);
                    if(maLoai.equals("Loai05")){
                        quanAoSoSinhList.add(sanPham);
                    }

                }
                adapterLoaiQASoSinh = new AdapterSanPham(getBaseContext(), quanAoSoSinhList);
                rcvLoaiQASoSinh.setAdapter(adapterLoaiQASoSinh);
            }
