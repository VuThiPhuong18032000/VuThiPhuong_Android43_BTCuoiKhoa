...
public class SanPhamBanChayActivity extends AppCompatActivity {
    RecyclerView rcvLoaiSPBanChay;
    AdapterSanPham adapterLoaiSanPhamBanChay;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_san_pham_ban_chay);
        rcvLoaiSPBanChay = findViewById(R.id.rcvLoaiSPBanChay);
        Button btnBackSPBC = findViewById(R.id.btnBackSPBC);
        RecyclerView.LayoutManager layoutManager = new GridLayoutManager(this,2,RecyclerView.VERTICAL,false);
        rcvLoaiSPBanChay.setLayoutManager(layoutManager);

        new DataGetPrduct().execute("https://demo7852048.mockable.io/SanPham");

        btnBackSPBC.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                startActivity(intent);
    ...
    class  DataGetPrduct extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String aVoid) {
            ...
            try {
                SanPham sanPham;
                List<SanPham> sanPhamBanChayList= new ArrayList<>();
...
//                    samPhamNoiBatList.add(samPham);
                    if(maLoai.equals("Loai02")){
                        sanPhamBanChayList.add(sanPham);
                    }

                }
                adapterLoaiSanPhamBanChay = new AdapterSanPham(getBaseContext(), sanPhamBanChayList);
                rcvLoaiSPBanChay.setAdapter(adapterLoaiSanPhamBanChay);
            }
